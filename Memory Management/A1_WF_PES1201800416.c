...
int size1;   //size of the entire array, essentially the parameter of alloc function
...
void allocate(int n)
{
    ...
    last_free_block->size_b = size1-sizeof(struct block);
...
{
    if (size <= 0 || size > size1 + sizeof(struct block))  //cannot have size 0 or negative
        return NULL;
    int *d;
    ...
    *max = 0;   //variable to keep track of size of biggest free block
    ...
    if(last_free_block != head){    //not first call to malloc
    ...
    for(temp = head; temp != NULL; temp = temp->next)   //loop to allocate size from max free block
    {
        if(temp->status == 0 && temp->size_b == *max)   //if free block exists
        {
            ...
            {
                if(*max == last_free_block->size_b && *max-size >= sizeof(struct block))
                {
                    last_free_block->size_b = *max - size - sizeof(struct block);
                    last_free_block = (struct block *)(((char*)last_free_block) + size + sizeof(struct block));
                }
    ...
    else    //first block being malloced
    {
        struct block *temp1 = (struct block*)p; //temp1 is the pointer to the memory being allocated in malloc function
        last_free_block = (struct block *)(((char*)last_free_block) + size+sizeof(struct block));   //move last free block pointer by (size+sizeof(struct block)) number of bytes
        last_free_block->size_b = size1 - size - 2*sizeof(struct block); //decrease size of last free block
        temp1=head+sizeof(struct block);    //increment the pointer to the allocated space
        temp1->size_b=size+sizeof(struct block);
        temp1->status = 1;
        last_free_block->status = 0;
        last_free_block->next = NULL;
        temp1->next=last_free_block;
        head->next = temp1;
        return temp1;
    }
...
void myfree(void *b)
{
    ...
    if(temp == b)   //if found
    {
        temp->size_b = temp->size_b - sizeof(struct block);   //decrease the book space ocuupied by the allocated block
    ...
    struct block *post = (temp->next)->next;    //an already existing free block right adjacent to the block just freed
    if(post->status == 0 && post != last_free_block)
    {
        temp->size_b += post->size_b + sizeof(struct block); 
        temp->next = temp1;
        temp1->next = (post->next)->next;
    }

    else if(post->status == 0 && post == last_free_block)
    {
        temp->size_b += post->size_b + sizeof(struct block);
        temp->next = NULL;
        last_free_block = temp;
        temp1 = NULL;
    }
    ...
    *s = 0; //counter variable to keep track of start address
    ...
    while(temp)
    {
        ...
        else
        {
            ...
            {
                if(temp->next && !((temp->next)->size_b == sizeof(struct block)))
                {
